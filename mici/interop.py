...
def convert_to_inference_data(traces, stats, energy_key="energy", lp_key="lp"):
    """Convert Mici `sample_chains` output to ArviZ `InferenceData` object.

    Args:
        traces (dict[str, list[array]]: Traces output from Mici `sample_chains` call.
            A dictionary of variables traced over sampled chains with the dictionary
            keys the variable names and the values a list of arrays, one array per
            sampled chain, with the first array dimension corresponding to the draw
            index and any remaining dimensions, the variable dimensions.
        stats (dict[str, list[array]]): Statistics output from Mici `sample_chains`
            call. A dictionary of chain statistics traced over sampled chains with the
            dictionary keys the statistics names and the values a list of arrays, one
            array per sampled chain, with the array dimension corresponding to the draw
            index.
        energy_key (Optional[str]): The key of an entry in the `traces` dictionary
            corresponding the value of the Hamiltonian energy for the accepted proposal
            (up to an additive constant). If present the corresponding values will be
            added to the `sample_stats` group of the returned `InferenceData` object.
        lp_key (Optional[str]): The key of an entry in the `traces` dictionary
            corresponding the value of the joint log posterior density for the model (up
            to an additive constant). If present the corresponding values will be added
            to the `sample_stats` group of the returned `InferenceData` object.

    Returns:
        arviz.InferenceData: ArviZ inference data object with traced chain data stored
            in the `posterior` group and additional chain statistics in the
            `sample_stats` group.
    """
    import arviz
...
def sample_pymc3_model(
    ...
    Args:
        model (Optional[pymc3.Model]): PyMC3 model defining posterior distribution to
            sample from. May be `None` if function is called from within model context
            manager.
        draws (int): The number of samples to draw.
        init (str): Initialization method to use. One of:
        ...
        trace (list): A list of variables to track. If `None`, `model.unobserved_RVs`
            is used.
        chains (int): The number of chains to sample. Running independent chains is
            important for some convergence statistics and can also reveal multiple modes
            in the posterior. If `None`, then set to either `cores` or 2, whichever is
            larger.
        cores (int): The number of chains to run in parallel. If `None`, set to the
            number of CPUs in the system, but at most 4.
        tune (int): Number of iterations to tune, defaults to 1000. Samplers adjust the
            step sizes, scalings or  similar during tuning. Tuning samples will be drawn
            in addition to the number specified in the `draws` argument, and will be
            discarded.
        progressbar (bool): Whether or not to display a progress bar.
        random_seed (Optional[int]): Seed for Numpy random number generator used for
            generating random variables while sampling chains. If `None` then generator
            will be seeded with entropy from operating system.
        jitter_max_retries (int): Maximum number of repeated attempts (per chain) at
            creating an initial matrix with uniform jitter that yields a finite
            probability. This applies to `jitter+adapt_diag` and `jitter+adapt_full`
            init methods.
        return_inferencedata (bool): Whether to return the traces as an
            `arviz.InferenceData` (True) object or a dict (False).

    Returns:
        Union[dict, arviz.InferenceData]: A dictionary or ArviZ `InferenceData` object
            containing the sampled chain output. Dictionary output (when
            `return_inferencedata=False`) has string keys corresponding to the name of
            each traced variable in the model, with the values being the corresponding
            values of the variables traced across the chains as NumPy arrays, with the
            first dimension the chain index  (of size equal to `chains`), the second
            dimension the draw index (of size equal to `draws`) and any remaining
            dimensions corresponding to the dimensions of the traced variable. If
            `return_inferencedata=True` an ArviZ `InferenceData` object is instead
            returned with traced chain data stored in the `posterior` group and
            additional chain statistics in the `sample_stats` group.
    """
...
def sample_stan_model(
    ...
    Args:
        ...
        data (dict[str, Any]): A Python dictionary or mapping providing the data for the
            model. Variable names are the keys and the values are their associated
            values.

    Kwargs:
        num_samples (int): A non-negative integer specifying the number of non-warm-up
            iterations per chain.
        num_warmup (int): A non-negative integer specifying the number of warm-up
            iterations per chain.
        ...
        metric (str): String specifying metric type. One of "unit_e", "diag_e" or
            "dense_e", indicating respectively to used a fixed identity matrix metric
        ...
        t0 (float): Adaptation iteration offset.
        ...
        seed (Optional[int]): Seed for Numpy random number generator used for generating
            random variables while sampling chains. If `None` then generator will be
    ...
    Returns:
        Union[dict, arviz.InferenceData]: A dictionary or ArviZ `InferenceData` object
            ...
            each traced variable in the model, with the values being the corresponding
            values of the variables traced across the chains as NumPy arrays, with the
            first dimension the flattened draw index across all chains (of size equal to
            `num_chains * num_samples`) and any remaining dimensions corresponding to
            the dimensions of the traced variable. If `return_inferencedata=True` an
            ArviZ `InferenceData` object is instead returned with traced chain data
            stored in the `posterior` group and additional chain statistics in the
            `sample_stats` group.
    """
