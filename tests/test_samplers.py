...
def test_memmaps_to_file_paths(tmp_path):
    ...
    mm = np.lib.format.open_memmap(
        filename, dtype=np.float64, mode='w+', shape=(1, 2))
...
def test_get_obj_size():
    obj = {'a': np.empty(1000, dtype=np.complex128),
           'b': np.empty(100, dtype=np.int8)}
...
class TestMarkovChainMonteCarloMethod(MarkovChainMonteCarloMethodTests):
    ...
    def sampler(self, integrator, system, rng):
        return mici.samplers.MarkovChainMonteCarloMethod(
            rng=rng, transitions={
                'momentum': mici.transitions.CorrelatedMomentumTransition(
                    system, 0.5),
                'integration':
                    mici.transitions.MetropolisStaticIntegrationTransition(
                        system, integrator, 1)
            })

    @pytest.fixture(
        params=('ChainState', 'dict'))
    ...
    @pytest.fixture(
        params=(None, (('integration', 'accept_stat'),)))
...
class HamiltonianMCMCTests(MarkovChainMonteCarloMethodTests):
    ...
    @pytest.fixture(
        params=(None, [], ['accept_stat', 'n_step']))
    def monitor_stats(self, request):
        return request.param
...
class TestStaticMetropolisHMC(HamiltonianMCMCTests):
    ...
    def test_max_tree_depth(self, sampler):
        ...
        assert sampler.transitions['integration_transition'].n_step == (
            new_n_step)
...
class TestRandomMetropolisHMC(HamiltonianMCMCTests):
    ...
    def sampler(self, integrator, system, rng):
        return mici.samplers.RandomMetropolisHMC(
            system=system, integrator=integrator, rng=rng,
...
class TestDynamicMultinomialHMC(DynamicHMCTests):
    ...
    def sampler(self, integrator, system, rng):
        return mici.samplers.DynamicMultinomialHMC(
            system=system, integrator=integrator, rng=rng,
            max_tree_depth=self.max_tree_depth, max_delta_h=self.max_delta_h)
