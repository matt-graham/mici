...
def test_state_pickling(state_vars):
    ...
    unpickled_state = pickle.loads(pickled_state)  # noqa: S301
...
def _mock_memoized_one_output_system_methods(system, state_vars):
    for var_name in state_vars:
        ...
        def func(state, var_name=var_name):
            return 2 * getattr(state, var_name)

        mocked_method = Mock(wraps=lambda _self, state: func(state), name=name)
    ...
    for var_names in combinations(state_vars.keys(), 2):
        ...
        def func(state, var_names=var_names):
            return getattr(state, var_names[0]) + getattr(state, var_names[1])
...
def _mock_memoized_aux_output_system_methods(system, state_vars):
    for var_name in state_vars:
        ...
        def aux_func(state, var_name=var_name):
            return -1 * getattr(state, var_name)

        def prim_func(state, var_name=var_name):
            return 2 * getattr(state, var_name)

        mocked_method = Mock(
            wraps=lambda _self, state: (prim_func(state), aux_func(state)),
            name=name,
        )
        mocked_aux_method = Mock(
            wraps=lambda _self, state: aux_func(state),
            name="aux_" + name,
        )
        bound_memoized_method = _bind_with_decorator(
            system,
    ...
    for var_names in combinations(state_vars.keys(), 2):
        ...
        def prim_func(state, var_names=var_names):
            return getattr(state, var_names[0]) + getattr(state, var_names[1])

        def aux_func(state, var_names=var_names):
            return -1 * getattr(state, var_names[0])
