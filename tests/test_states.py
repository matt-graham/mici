...
def _mock_memoized_one_output_system_methods(system, state_vars):
    for var_name in state_vars.keys():
        ...
        mocked_method = Mock(wraps=lambda self, state: func(state), name=name)
        bound_memoized_method = _bind_with_decorator(
            system, name, mocked_method, mici.states.cache_in_state(var_name)
        )
    ...
    for var_names in combinations(state_vars.keys(), 2):
        ...
        mocked_method = Mock(wraps=lambda self, state: func(state), name=name)
        bound_memoized_method = _bind_with_decorator(
            system, name, mocked_method, mici.states.cache_in_state(*var_names)
        )
...
def _mock_memoized_aux_output_system_methods(system, state_vars):
    for var_name in state_vars.keys():
        ...
        mocked_method = Mock(
            wraps=lambda self, state: (prim_func(state), aux_func(state)), name=name
        )
